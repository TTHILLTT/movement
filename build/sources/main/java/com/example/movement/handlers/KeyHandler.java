...
public class KeyHandler {
    ...
    if (player != null) {
        ...
        if (Keyboard.isKeyDown(airJumpKey.getKeyCode())) {
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (MovementMod.flyEnabled && Keyboard.isKeyDown(Keyboard.KEY_SPACE)) {
            Minecraft.getMinecraft().thePlayer.motionY = 0.5; // 自定义飞行上升速度
        }
        ...
        // 空中跳跃逻辑（带反作弊保护）
        if (MovementMod.airJumpEnabled && airJumpKey.isKeyDown()) {
            if (jumpTimer <= 0 && !event.player.onGround) {
                // 模拟合法跳跃数据
                event.player.motionY = 0.42 * MovementMod.jumpBoost * AntiCheatHelper.getRandomizedFactor();
                
                // 添加位置随机偏移
                double[] offset = AntiCheatHelper.getPositionOffset();
                event.player.setPosition(
                    event.player.posX + offset[0],
                    event.player.posY + offset[1],
                    event.player.posZ + offset[2]
                );
